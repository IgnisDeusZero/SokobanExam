...
function updateField(game) {
    if (game) {
        scoreElement.innerText = `Your moves: ${game.score}`;
        startMessage.innerText = `Your moves: ${game.score}. Again?`;
    }
    setTimeout(
        () => {
...
function initializePage() {
    ...
    for (var i = 0; i < startButtons.length; i++) {
        addListenerToStart(startButtons[i]);
    }
...
function addListenerToStart(button) {
